...
- (void)animateOpen {
    ...
    CGFloat newWidth = kSubviewHorizontalMargin + self.bounds.size.height;
    
    for(UIView *view in _optionViews) {
        newWidth += view.frame.size.height + kSubviewHorizontalMargin;
    }
    ...
    for(UIView *view in _optionViews)
    {
        ...
        view.center = CGPointMake(CGRectGetMidX(self.bounds),curX + floorf(view.frame.size.height*0.5f));
        ...
        curX += view.frame.size.height + kSubviewHorizontalMargin;
    }
    
    //Expand the bg,设置背景的宽高
    [self setBoundsForLayer:_backgroundGradientLayer newBounds:CGRectMake(0, 0, _backgroundGradientLayer.bounds.size.height,newWidth) duration:kAnimationDuration];
    ...
    CGPoint position = CGPointMake(CGRectGetMidX(self.bounds),_backgroundGradientLayer.position.y + directionModifier*0.5f*(_backgroundGradientLayer.bounds.size.height - self.bounds.size.width)
);
